...
class MultiWalkerEnv:
    ...
    def __init__(
        ...
        angle_noise=1e-3,
        forward_reward=1.0,
        walker_forward_reward=1.0,
        out_of_package_reward=200,
        location_reward = 200,
        ...
        self.walker_forward_reward = walker_forward_reward
        ...
        self.out_of_package_reward = out_of_package_reward
        self.location_reward = location_reward
        self.seed_val = None
        self._seed()
        self.setup()
        self.screen = None
    ...
    def setup(self):
        ...
        self.prev_out_of_package_range_shaping = np.zeros(self.n_walkers) # new shaping
        self.prev_location_shaping = np.zeros(self.n_walkers) # new shaping
        self.pre_walker_forward_shaping = np.zeros(self.n_walkers) # new shaping
    ...
    def scroll_subroutine(self):
        ...
        for i in range(self.n_walkers):
            ...
            # xd = (self.package.position.x - x) / self.package_length # the original formula is not correct
            xd = (x - (self.package.position.x - self.package_length / 2)) / self.package_length
            ...
            # if 0.0 <= neighbor_obs[4] <= 1.0:
            #     shaping = -5.0 * abs(walker_obs[0])
            #     rewards[i] = shaping - self.prev_shaping[i]
            #     self.prev_shaping[i] = shaping
            # else:
            #     rewards[i] = 0
            #     self.prev_shaping[i] = 0

            # # Check
            # print(f"Walker identity: {i}")
            # print(f"Package x-position: {self.package.position.x}")
            # print(f"Walker x-position: {x}")
            # print(f"Package length: {self.package_length}")
            # print(f"Calculated neighbor_obs[4]: {xd}")

            """
            Adding new shaping functions
            """

            # New shaping reward: walker forward
            ...
            # New shaping reward: under package
            ...
            # New shaping reward: location
            # Each walker is encouraged to stay within a small range of package
            # lower_bound = i / self.n_walkers
            # upper_bound = (i + 1) / self.n_walkers
            # if neighbor_obs[4] < lower_bound:  # Too far behind
            #     location_shaping = self.location_reward * (neighbor_obs[4] - lower_bound)  # Negative reward
            # elif neighbor_obs[4] > upper_bound:  # Too far ahead
            #     location_shaping = self.location_reward * (upper_bound - neighbor_obs[4])  # Negative reward
            # else:
            #     location_shaping = 0  # No penalty if within range
            # rewards[i] += location_shaping - self.prev_location_shaping[i]
            # self.prev_location_shaping[i] = location_shaping

        # # New shaping function for distance between first and last walkers
        # # Calculate the x-positions of the first and last walkers
        # first_walker_x = self.walkers[0].hull.position.x if self.walkers[0].hull else 0
        # last_walker_x = self.walkers[-1].hull.position.x if self.walkers[-1].hull else 0
        # distance_between_walkers = abs(first_walker_x - last_walker_x)
        # max_distance = self.package_length * 1 # Adjust multiplier as needed
        # separation_penalty_coefficient = 50
        # if distance_between_walkers > max_distance:
        #     separation_penalty = separation_penalty_coefficient * (distance_between_walkers - max_distance)    
        #     # Penalize the walkers for being too far away
        #     rewards[0] -= separation_penalty
        #     rewards[-1] -= separation_penalty

        """
        End of adding new shaping functions
        """
